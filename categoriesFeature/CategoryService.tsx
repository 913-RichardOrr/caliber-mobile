...
class categoryService {
    ...
    getCategories(token: string, queryString?: boolean): Promise<any> {
        return axios.get(`${this.URI}/categories`, {params: { active: queryString}, headers: { Authorization: `Bearer ${token}`}}).then((result) => {
            return result.data;
        }).catch((err) => {
    ...
    addCategory(token: string, skill: string): Promise<any> {
        return axios.post(`${this.URI}/categories`, skill, {headers: { Authorization: `Bearer ${token}`}}).then((result) => {
            return result.data;
        }).catch((err) => {
    ...
    updateCategory(token: string, category: Category): Promise<any>{ 
        ...
        return axios.put(`${this.URI}/categories` + `/${category.categoryid}`, category, {headers: { Authorization: `Bearer ${token}`}}).then((result) => {
            return result.data;
        }).catch((err) => {
            console.log(`Error getting category: ${err}`);
            return null;
