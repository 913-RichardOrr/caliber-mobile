...
class BatchWeekService {
	...
	constructor() {
		this.URI =
			'https://kx49u9u25h.execute-api.us-east-1.amazonaws.com/default/';
	}
	...
	getWeeksByBatchId(token: string, batchid: string): Promise<QcWeek[]> {
		return axios
			.get(this.URI + `qc/batches/${batchid}/weeks`, this.getConfig(token))
	...
	addWeek(token: string, qw: QcWeek): Promise<any> {
		return axios
			.post(
				this.URI + `qc/batches/${qw.batchid}/weeks`,
				qw,
